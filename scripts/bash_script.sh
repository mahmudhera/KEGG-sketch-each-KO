...
echo sourmash sketch translate -f -p scaled=1000,k=5 ${DATADIR}/${FILEPREFIX}*.fastq -o ${SIGFILE} --merge ${SIGFILE}
sourmash sketch translate -f -p scaled=1000,k=5 ${DATADIR}/${FILEPREFIX}*.fastq -o ${SIGFILE} --merge ${SIGFILE}
echo sourmash gather --protein -k 5 --estimate-ani-ci --threshold-bp 500 ${SIGFILE} ${REF} -o ${OUTDIR}/${FILEPREFIX}_$(basename $REF)_gather_k_5.csv
